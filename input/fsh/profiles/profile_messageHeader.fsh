...
* status[x] ^short = "此訊息代表的事件的代碼或連結到事件定義"
* status[x] ^binding.description = "可參考所綁定值集，但此值集只是針對這個欄位的一個可能值的範例，不預期也不鼓勵使用者一定要使用此值集的代碼。"
* status[x] ^definition = "用於識別此訊息所代表的事件並將其與其定義連接的代碼。作為FHIR規範一部分定義的事件，其系統值（system value）為「http://terminology.hl7.org/CodeSystem/message-statuss」。或者是指向statusDefinition的URI。"
* status[x] ^comment = "事件的時間將在foscus resource中找到。訊息的時間將在[Bundle.timestamp](bundle-definitions.html#Bundle.timestamp)中找到。"
* status[x] ^requirements = "驅動與此訊息相關的行為"
