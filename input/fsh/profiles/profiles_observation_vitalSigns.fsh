Profile:        TWCoreObservationVitalSigns
...
//* component.value[x] from http://hl7.org/fhir/ValueSet/ucum-vitals-common
...
* category[VSCat] ^short = "檢驗檢查分類"
* category[VSCat] ^binding.description = "高層次的檢驗檢查分類；鼓勵使用所綁定值集中的代碼，但不強制一定要使用此值集，你也可使用其他值集的代碼或單純以文字表示。"
* category[VSCat] ^definition = "可以正式參照一個專門術語或本體論來定義一個概念，或者也可以文字表述此概念。"
* category[VSCat] ^requirements = "用於篩選取得及顯示之檢驗檢查"
* category[VSCat] ^comment = "並非所有的專門術語使用都符合這個一般模式。在某些情況下，資料模型不應該使用CodeableConcept而應直接使用Coding，並提供他們自己的結構來管理text、codings、translation、資料項目之間的關係、以及先組合配對／前組合式（pre-coordination）和後組合配對／後組合式（post-coordination）。"

/*
* category[VSCat].id ^short = "唯一可識別ID，以供資料項目間相互參照。"
* category[VSCat].id ^definition = "resource中資料項目的唯一ID（用於內部參照）。這可以是任何不含空格的字串。"
* category[VSCat].extension ^short = "擴充的資料項目"
* category[VSCat].extension ^definition = "可用於表示不屬於此資料項目基本定義的附加資訊。為了擴充的使用安全和可管理，對擴充的定義和使用有一套嚴格的管理。儘管任何實作者都可以定義一個擴充，但作為擴充定義的一部分，有一套要求 **必須（SHALL）** 滿足。"
* category[VSCat].extension ^comment = "無論使用或定義擴充的機構或管轄區，任何應用程式、專案或標準使用擴充都不背負任何汙名（stigma）。使用擴充是允許FHIR規範為每個人保留一個核心的簡易性。"
*/

* category[VSCat].coding ^short = "由專門術語系統（terminology system）定義的代碼"
* category[VSCat].coding ^definition = "由專門術語系統（terminology system）所定義之代碼的參照。"
* category[VSCat].coding ^requirements = "允許使用者在不知道識別碼系統的情況下利用識別碼"
* category[VSCat].coding ^comment = "代碼可以在列舉清單（enumerations）或代碼清單（code lists）中非常隨意地定義，直至有非常正式的定義，如SNOMED CT—更多資訊見HL7 v3核心原則（Core Principles）。編碼的排序是未定義的因而 **必須沒有（SHALL NOT）** 被用來推斷意義。一般來說，最多只有一個編碼值（coding values）會被標記為UserSelected = true。"

/*
* category[VSCat].coding.id ^short = "唯一可識別ID，以供資料項目間相互參照。"
* category[VSCat].coding.id ^definition = "resource中資料項目的唯一ID（用於內部參照）。這可以是任何不含空格的字串。"
* category[VSCat].coding.extension ^short = "擴充的資料項目"
* category[VSCat].coding.extension ^definition = "可用於表示不屬於此資料項目基本定義的附加資訊。為了擴充的使用安全和可管理，對擴充的定義和使用有一套嚴格的管理。儘管任何實作者都可以定義一個擴充，但作為擴充定義的一部分，有一套要求 **必須（SHALL）** 滿足。"
* category[VSCat].coding.extension ^comment = "無論使用或定義擴充的機構或管轄區，任何應用程式、專案或標準使用擴充都不背負任何汙名（stigma）。使用擴充是允許FHIR規範為每個人保留一個核心的簡易性。"
*/

* category[VSCat].coding.system ^short = "專門術語系統（terminology system）的識別"
* category[VSCat].coding.system ^definition = "定義代碼中符號意義的代碼系統識別。"
* category[VSCat].coding.system ^requirements = "需要明確說明符號定義的來源"
* category[VSCat].coding.system ^comment = "URI可以是一個OID（urn:oid:...）或一個UUID（urn:uuid:...）；OID和UUID **必須（SHALL）** 參照HL7 OID註冊中心；否則，URI應該來自HL7的FHIR定義的特殊URI列表，或者它應該參照一些明確建立的系统定義。"
* category[VSCat].coding.version ^short  = "系統的版本—如果相關的話"
* category[VSCat].coding.version ^definition  = "選擇此代碼時使用的代碼系統版本；請注意，一個維護良好的代碼系統不需要版本報告，因為代碼的意義在不同系統版本中是一致的；然而，不能始終保證這點，當不能保證意義一致時， **必須（SHALL）** 將版本資訊也一併作交換。"
* category[VSCat].coding.version ^comment  = "如果專門術語沒有明確定義應該使用什麼字串來識別代碼系統的版本，建議使用版本正式發布的日期（用FHIR日期格式表示）作為版本日期。"
* category[VSCat].coding.code ^short  = "系統定義的語法之符號"
* category[VSCat].coding.code ^definition  = "系統定義的語法之符號；符號可能是一個預先定義的代碼，也可能是代碼系統定義的語法中的表達式（如後組合配對／後組合式）。"
* category[VSCat].coding.code ^requirements  = "需要參照系統中的一個特定代碼"
* category[VSCat].coding.display ^short  = "由系統定義的表示法"
* category[VSCat].coding.display ^definition  = "遵循系統的規則以呈現代碼含義的表示法"
* category[VSCat].coding.display ^requirements  = "需要能為不了解此系統的讀者呈現可讀的代碼含義"
* category[VSCat].coding.userSelected ^short = "此編碼是否由使用者直接選擇？"
* category[VSCat].coding.userSelected ^definition = "表明此編碼是由使用者直接選擇，例如：從可用項目（代碼或顯示名稱）的清單中選擇。"
* category[VSCat].coding.userSelected ^requirements = "已被確定為一個臨床安全準則—此確切的系統／代碼對(code pair)是被明確選擇的，而不是由系統根據一些規則或是程式語言處理判斷。"
* category[VSCat].coding.userSelected ^comment = "在一系列備選方案中，直接選擇的代碼是新翻譯最合適的起點；關於「直接選擇」的確切意義，存在模糊不清之處，可能需要貿易夥伴的同意，以更完整澄清此資料項目的使用及其後果。"
* category[VSCat].text ^short = "概念的文字表示法"
* category[VSCat].text ^definition = "輸入資料的使用者所見／所選／所說的人類可讀文字表述，和（或）其代表使用者的預期含義。"
* category[VSCat].text ^requirements = "專門術語中的代碼並不總是能捕捉人類使用的細微差別的正確意義，或者根本就沒有合適的代碼；這些情況下，文字表述被用來捕捉來源的全部意義。"
* category[VSCat].text ^comment = "很多時候，此文字表述與其中一個代碼的顯示名稱相同。"

* code ^short = "概念（concept）－參照一個專門術語或只是純文字。"
//* code ^binding.description = "應填入[生命體徵值集](https://twcore.mohw.gov.tw/ig/twcore/ValueSet-vital-signs-tw.html)值集中的其中一個代碼。"
* code ^binding.description = "應填入所綁定值集中的其中一個代碼。"
...
* value[x] ^binding.description = "用於記錄生命體徵的常用UCUM單元；應填入所綁定值集中適合的代碼，確定無適合的代碼才可以使用其他值集的代碼來表示。"
* value[x] ^definition = "如果此資訊有一個簡單的值，則將其作為此項檢驗檢查的結果。"
* value[x] ^requirements = "通常一項檢驗檢查就會有一個值，雖然若它有錯誤或者它代表一組檢驗檢查可能不會有值。"
* value[x] ^comment = "一項檢驗檢查可能有：1）單一值、2）一個值及一組相關或組成之檢驗檢查的多個值、或者3）只有一組相關或組成之檢驗檢查的多個值。如果有一個值，這個資料項目的datatype應該由Observation.code決定。如果這個資料項目是編碼或者如果與Observation.code相關的type定義一個編碼值，那麼將使用只有文字表述的CodeableConcept來代替字串。若需要更多的說明，請參閱下面Notes小節。"
...
* bodySite ^binding.description = "代碼說明解剖學位置，可能包含側邊；可參考所綁定值集，但此值集只是針對這個欄位的一個可能值的範例，不預期也不鼓勵使用者一定要使用此值集的代碼。"
...
* method ^binding.description = "檢驗檢查的方法；可參考所綁定值集，但此值集只是針對這個欄位的一個可能值的範例，不預期也不鼓勵使用者一定要使用此值集的代碼。"
...
* component.code ^binding.description = "代碼指明檢驗檢查的名稱；可參考所綁定值集，但此值集只是針對這個欄位的一個可能值的範例，不預期也不鼓勵使用者一定要使用此值集的代碼。"
...
* component.value[x] ^binding.description = "用於記錄生命體徵的常用UCUM單元。；應填入[VitalSignsUnits](http://hl7.org/fhir/R4/valueset-ucum-vitals-common.html)值集中適合的代碼，確定無適合的代碼才可以使用其他值集的代碼來表示。"
