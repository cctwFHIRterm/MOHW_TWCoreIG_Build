...
* statusHistory ^short = "生命週期中感興趣的事件清單"
* statusHistory ^definition = "連結至Provenance resource的過去版本，或履行請求或事件resource的Provenance紀錄，其指明可能與查看當前版本resource的使用者相關的關鍵狀態之轉換或更新。"
* statusHistory ^comment = "這可能不包括請求的所有版本的出處–只包括那些被認為「相關」或重要的版本。這 **必須沒有（SHALL NOT）** 包括與此resource的當前版本相關的出處。（如果此出處被認為是一個「相關」的變化，它將需要作為後續更新的一部分。在此之前，可使用_revinclude直接查詢指向此版本的出處。 所有出處（Provenances）都應有這個藥品請求請求的一些歷史版本作為對象。）"
